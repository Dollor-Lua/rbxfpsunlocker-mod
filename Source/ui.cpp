...
#define RFU_TRAYMENU_MGITHUB	WM_APP + 8
#define RFU_TRAYMENU_STUDIO		WM_APP + 9
#define RFU_TRAYMENU_CFU		WM_APP + 10
#define RFU_TRAYMENU_ADV_NBE	WM_APP + 11
#define RFU_TRAYMENU_ADV_SE		WM_APP + 12
#define RFU_TRAYMENU_ADV_QS		WM_APP + 13
#define RFU_TRAYMENU_CLIENT		WM_APP + 14
#define RFU_TRAYMENU_STARTUP	WM_APP + 15

#define RFU_FCS_FIRST			(WM_APP + 20)
#define RFU_FCS_NONE			RFU_FCS_FIRST + 0
#define RFU_FCS_1				RFU_FCS_FIRST + 1
#define RFU_FCS_15				RFU_FCS_FIRST + 2
#define RFU_FCS_30				RFU_FCS_FIRST + 3
#define RFU_FCS_60				RFU_FCS_FIRST + 4
#define RFU_FCS_75				RFU_FCS_FIRST + 5
#define RFU_FCS_120				RFU_FCS_FIRST + 6
#define RFU_FCS_144				RFU_FCS_FIRST + 7
#define RFU_FCS_240				RFU_FCS_FIRST + 8
#define RFU_FCS_600				RFU_FCS_FIRST + 9
#define RFU_FCS_LAST			(RFU_FCS_600)
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case RFU_TRAYICON:
	{
		if (lParam == WM_RBUTTONDOWN || lParam == WM_LBUTTONDOWN)
		{
			...
			// AppendMenu(submenu, MF_STRING, RFU_FCS_1, "1");
			AppendMenu(submenu, MF_STRING, RFU_FCS_15, "15");
			...
			AppendMenu(submenu, MF_STRING, RFU_FCS_600, "600");
			...
			AppendMenu(popup, MF_STRING | (ConsoleEnabled ? MF_CHECKED : 0), RFU_TRAYMENU_CONSOLE, "Toggle Console");
			AppendMenu(popup, MF_STRING | (Settings::RunOnStartup ? MF_CHECKED : 0), RFU_TRAYMENU_STARTUP, "Toggle Run on Startup");
			AppendMenu(popup, MF_SEPARATOR, -2, NULL);

			AppendMenu(popup, MF_STRING, RFU_TRAYMENU_GITHUB, "Visit axstin's GitHub (OG)");
			AppendMenu(popup, MF_STRING, RFU_TRAYMENU_MGITHUB, "Visit GitHub (MOD)");

			AppendMenu(popup, MF_SEPARATOR, -3, NULL);
			...
			if (result != 0)
			{
				...
				case RFU_TRAYMENU_CONSOLE:
					UI::ToggleConsole();
					CheckMenuItem(popup, RFU_TRAYMENU_STUDIO, ConsoleEnabled ? MF_CHECKED : MF_UNCHECKED);
					break;
				...
				case RFU_TRAYMENU_MGITHUB:
					ShellExecuteA(NULL, "open", "https://github.com/" RFU_MGITHUB_REPO, NULL, NULL, SW_SHOWNORMAL);
					break;
				...
				case RFU_TRAYMENU_STARTUP:
					...
					CheckMenuItem(popup, RFU_TRAYMENU_STARTUP, Settings::RunOnStartup ? MF_CHECKED : MF_UNCHECKED);
					if (Settings::RunOnStartup) {
						AddToStartupPrograms();
						CheckMenuItem(popup, RFU_TRAYMENU_STARTUP, MF_CHECKED);
					}
					else {
						RemoveFromStartupPrograms();
						CheckMenuItem(popup, RFU_TRAYMENU_STARTUP, MF_UNCHECKED);
					}
					break;
				...
				default:
					if (result >= RFU_FCS_FIRST
						&& result <= RFU_FCS_LAST)
					{
						static double fcs_map[] = { 0.0, 1.0, 15.0, 30.0, 60.0, 75.0, 120.0, 144.0, 240.0, 600.0 };
...
bool UI::UpdateHotkey()
{
	if (RegisterHotKey(
		...
		0x51))  //0x51 is 'Q' 
	{
		std::cout << "Registered hotkey CTRL + " << "Q" << std::endl;
		return true;
	}
