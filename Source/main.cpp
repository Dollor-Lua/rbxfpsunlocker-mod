...
std::vector<std::string> Split(std::string str, std::string split) {
	...
	std::vector<std::string> arr(arsize);
	size_t prepos = 0;
	pos = str.find(split);
	int incounters = -1;
	while (pos != std::string::npos) {
		incounters++;
		std::string value = str.substr(prepos, pos);
		arr.push_back(value);
		prepos = pos;
		pos = str.find(split, prepos + 1);
	}
	return arr;
}
...
{
	printf("Adding to startup programs...\n");

	TCHAR szPath[MAX_PATH];
	GetModuleFileName(NULL, szPath, MAX_PATH);
	HKEY newValue;
	RegOpenKey(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Run", &newValue);
	RegSetValueEx(newValue, "rbxfpsunlocker", 0, REG_SZ, (LPBYTE)szPath, sizeof(szPath));
	RegCloseKey(newValue);
}

void RemoveFromStartupPrograms()
{
	printf("Removing from startup programs...\n");
	RegDeleteValue(HKEY_CURRENT_USER, "rbxfpsunlocker");
}
