...
{
	std::string response;
	if (!HttpRequest("https://api.github.com/repos/" RFU_MGITHUB_REPO "/releases/latest", response))
	{
	...
	if (latest_version != RFU_VERSION)
	{
		...
		{
			ShellExecuteA(NULL, "open", "https://github.com/" RFU_MGITHUB_REPO "/releases", NULL, NULL, SW_SHOWNORMAL);
			return true;
		}
	}
